...
# ---------------------------------------------------------------------------
# Hardware Fingerprint License Check
# Runs on startup to ensure this copy is authorized for this machine.
# Skipped during automated testing (set RESTAURANT_POS_TESTING=1 to bypass).
# ---------------------------------------------------------------------------
if not os.environ.get("RESTAURANT_POS_TESTING"):
    import license_check
    license_check.verify()
...
# When running as a PyInstaller EXE, app.static_folder points to a temp
# extraction folder that is deleted on exit. We instead save uploaded files
# next to the EXE (or next to app.py during development) so they survive
# server restarts.
# ---------------------------------------------------------------------------
if getattr(sys, 'frozen', False):
    # Running as EXE — use the folder that contains the EXE
    _BASE_DIR = os.path.dirname(sys.executable)
else:
    # Running as plain Python script
    _BASE_DIR = os.path.dirname(os.path.abspath(__file__))

PERSISTENT_UPLOAD_DIR = os.path.join(_BASE_DIR, 'uploads')
...
def init_db():
    ...
    # Add kitchen_printed column to existing order_items table if it doesn't exist
    try:
        cursor.execute('ALTER TABLE order_items ADD COLUMN kitchen_printed INTEGER DEFAULT 0')
    except sqlite3.OperationalError:
        pass
...
def api_create_order():
    ...
    try:
        ...
        if settings_dict.get('enable_kitchen_print') == 'true':
            ...
            # Use threaded version to prevent blocking
            print_kitchen_ticket_threaded(order_id, new_item_ids)
        else:
...
def print_kitchen_ticket_threaded(order_id, new_item_ids=None):
    """Run kitchen printing in a separate thread to avoid blocking"""
    def run_printing():
        try:
            # Create a new connection for this thread
            thread_conn = get_db_connection()
            print_kitchen_ticket(order_id, thread_conn, new_item_ids)
            thread_conn.close()
        except Exception as e:
            print(f"Error in threaded kitchen printing: {e}")

    thread = threading.Thread(target=run_printing)
    thread.daemon = True
    thread.start()
...
def serve_upload(filename):
    ...
    return send_from_directory(PERSISTENT_UPLOAD_DIR, filename)
    ...
    try:
        ...
        if 'logo' in request.files:
            ...
            if logo_file and logo_file.filename:
                ...
                # Generate unique filename and save to persistent uploads dir
                import uuid
                filename = f"logo_{uuid.uuid4().hex}.{file_extension}"
                filepath = os.path.join(PERSISTENT_UPLOAD_DIR, filename)
                
                # Save the file
                logo_file.save(filepath)
                # Use /uploads/ route (served by serve_upload above)
...
def api_reports():
    try:
        ...
        payment_method_filter = request.args.get('payment_method')
        waiter_id_filter = request.args.get('waiter_id')
        ...
        if waiter_id_filter:
            where_clauses.append("o.waiter_id = ?")
            params.append(waiter_id_filter)
        ...
        query_revenue = f'''
            ...
            WHERE {where_sql}
        '''
        total_revenue = conn.execute(query_revenue, params).fetchone()['revenue']
        
        # 2. Total orders for the period
        # Note: We join payments to filter by payment method if needed
        query_orders = f'''
            ...
            WHERE {where_sql}
        '''
        total_orders = conn.execute(query_orders, params).fetchone()['count']
        ...
        query_items = f'''
            ...
            WHERE {where_sql}
        '''
        total_items = conn.execute(query_items, params).fetchone()['items']
        ...
        # For trend, we need to apply filters including the specific date
        trend_where = where_sql.replace("DATE(o.created_at) BETWEEN ? AND ?", "DATE(o.created_at) = ?")
        
        while current_date <= end_date_obj:
            date_str = current_date.strftime('%Y-%m-%d')
            trend_params = [date_str] + params[2:] # params[0] and [1] are date range, so skip them
        ...
        # If filtering by payment method, this chart will show 100% of that method, which is correct behavior
        query_payment_methods = f'''
            SELECT p.payment_method, COUNT(DISTINCT o.id) as count, SUM(p.total_amount) as total
            FROM payments p
            JOIN orders o ON p.order_id = o.id
            WHERE {where_sql}
            GROUP BY p.payment_method
        '''
        payment_methods = conn.execute(query_payment_methods, params).fetchall()
        ...
        query_top_items = f'''
            ...
            LIMIT 10
        '''
        top_items = conn.execute(query_top_items, params).fetchall()
        
        # 7. Waiter performance (Enhanced)
        # We need to construct a specific WHERE clause for this one because it joins differently
        # and has base conditions like w.active=1
        
        waiter_where_parts = ["DATE(o.created_at) BETWEEN ? AND ?"]
        waiter_params = [start_date, end_date]
        
        if payment_method_filter:
            waiter_where_parts.append("p.payment_method = ?")
            waiter_params.append(payment_method_filter)
            
        if waiter_id_filter:
            waiter_where_parts.append("w.id = ?")
            waiter_params.append(waiter_id_filter)
            
        waiter_where_sql = " AND ".join(waiter_where_parts)
        ...
        query_waiter_perf = f'''
            SELECT 
                ...
                COALESCE(SUM(CASE WHEN p.payment_method = 'cash' THEN p.total_amount ELSE 0 END), 0) as cash_revenue,
                COALESCE(SUM(CASE WHEN p.payment_method = 'card' THEN p.total_amount ELSE 0 END), 0) as card_revenue
            FROM waiters w
            LEFT JOIN orders o ON w.id = o.waiter_id
            LEFT JOIN payments p ON o.id = p.order_id
            WHERE w.active = 1 AND {waiter_where_sql}
            GROUP BY w.id, w.name
            ORDER BY revenue DESC
        '''
        waiter_performance = conn.execute(query_waiter_perf, waiter_params).fetchall()
        
        # 8. Table performance
        # Similar logic for tables
        table_where_parts = ["DATE(o.created_at) BETWEEN ? AND ?"]
        table_params = [start_date, end_date]
        
        if payment_method_filter:
            table_where_parts.append("p.payment_method = ?")
            table_params.append(payment_method_filter)
            
        if waiter_id_filter:
            table_where_parts.append("o.waiter_id = ?")
            table_params.append(waiter_id_filter)
            
        table_where_sql = " AND ".join(table_where_parts)
        ...
        query_table_perf = f'''
            ...
            LEFT JOIN payments p ON o.id = p.order_id
            WHERE t.active = 1 AND {table_where_sql}
            GROUP BY t.id, t.table_number
            ORDER BY revenue DESC
        '''
        table_performance = conn.execute(query_table_perf, table_params).fetchall()
        
        # 9. Hourly sales (for today only if single day selected)
        hourly_sales = []
        if start_date == end_date:
            hourly_where = where_sql + " AND CAST(strftime('%H', o.created_at) AS INTEGER) = ?"
            
            for hour in range(24):
                hour_params = params + [hour]
                hour_revenue = conn.execute(f'''
                    SELECT COALESCE(SUM(p.total_amount), 0) as revenue
                    FROM payments p
                    JOIN orders o ON p.order_id = o.id
                    WHERE {hourly_where}
                ''', hour_params).fetchone()['revenue']
...
# Allows viewing reports from a selected .db file without restoring it.
# ---------------------------------------------------------------------------

def get_backup_connection(db_path):
    """Helper to connect to a specific database file."""
...
@app.route('/backup-explorer')
@login_required
def backup_explorer():
    """Render the Backup Explorer page."""
    db_path = request.args.get('db_path')
    if not db_path:
        return redirect(url_for('settings', _anchor='backup'))
    
    filename = os.path.basename(db_path)
    return render_template('backup_explorer.html', db_path=db_path, filename=filename)

@app.route('/api/backup-explorer/stats')
@login_required
def api_backup_explorer_stats():
    """Get stats from a backup database file with optional filtering."""
    db_path = request.args.get('db_path')
    if not db_path:
    ...
    try:
        conn = get_backup_connection(db_path)
        if not conn:
            return jsonify({'error': 'Database file not found'}), 404
        
        # Base WHERE clauses for filtering
        where_clauses = ["1=1"] # Always true
        params = []
        
        if start_date and end_date:
            where_clauses.append("DATE(o.created_at) BETWEEN ? AND ?")
        ...
        # 0. Get all waiters for filter dropdown
        query_all_waiters = 'SELECT id, name FROM waiters ORDER BY name ASC'
        res['waiters'] = [dict(row) for row in conn.execute(query_all_waiters).fetchall()]
        ...
        query_revenue = f'''
            ...
            WHERE {where_sql}
        '''
        res['totalRevenue'] = conn.execute(query_revenue, params).fetchone()['revenue']
        ...
        query_orders = f'''
            ...
            WHERE {where_sql}
        '''
        res['totalOrders'] = conn.execute(query_orders, params).fetchone()['count']
        ...
        query_items = f'''
            ...
            WHERE {where_sql}
        '''
        res['totalItems'] = conn.execute(query_items, params).fetchone()['items']
        
        # 4. Average order value
        res['avgOrderValue'] = res['totalRevenue'] / res['totalOrders'] if res['totalOrders'] > 0 else 0
        
        # 5. Revenue Trend (Daily)
        # For trend, we use paid_at or created_at. Let's stay consistent with created_at for filtering.
        query_trend = f'''
            SELECT DATE(o.created_at) as date, SUM(p.total_amount) as revenue 
            FROM payments p
            JOIN orders o ON p.order_id = o.id
            WHERE {where_sql}
            ...
            ORDER BY date ASC
        '''
        res['revenueTrend'] = [dict(row) for row in conn.execute(query_trend, params).fetchall()]
        ...
        query_methods = f'''
            ...
            GROUP BY p.payment_method
        '''
        res['paymentMethods'] = [dict(row) for row in conn.execute(query_methods, params).fetchall()]
        ...
        query_top_items = f'''
            ...
            LIMIT 10
        '''
        res['topItems'] = [dict(row) for row in conn.execute(query_top_items, params).fetchall()]
        
        # 8. Waiter Performance
        query_waiters = f'''
            SELECT w.name, COUNT(DISTINCT o.id) as orders, SUM(p.total_amount) as revenue
            FROM waiters w
            ...
            ORDER BY revenue DESC
        '''
        res['waiterPerformance'] = [dict(row) for row in conn.execute(query_waiters, params).fetchall()]
...
@app.route('/api/backup/upload-temp', methods=['POST'])
@login_required
def upload_backup_temp():
    """Temporarily upload a backup file for exploration."""
    ...
    if file and file.filename.endswith('.db'):
        import uuid
        # Save to backups folder with a temp prefix to distinguish
        filename = f"temp_explore_{uuid.uuid4().hex[:8]}_{secure_filename(file.filename)}"
    ...
    return jsonify({'success': False, 'error': 'Invalid file type. Only .db files are allowed.'}), 400
...
@login_required
def clear_all_data():
    """Clear selected data types while handling foreign key constraints"""
    try:
        data = request.get_json() or {}
        types = data.get('types', [])
        
        # If no types specified, default to clearing everything (legacy behavior or safety fallback)
        # But frontend should send types. If empty list, do nothing.
        if not types:
             return jsonify({'success': False, 'error': 'No data types selected'}), 400

        conn = get_db_connection()
        cursor = conn.cursor()
        
        # 1. Orders (Clear history)
        if 'orders' in types:
        ...
        # 2. Foods (Clear food menu items)
        if 'foods' in types:
            # First delete order items referencing these foods (if orders weren't already cleared)
            if 'orders' not in types:
                cursor.execute('''
                    DELETE FROM order_items 
                    WHERE menu_item_id IN (SELECT id FROM menu_items WHERE category != 'drink' AND category != 'beverage')
                ''')
            # Now delete the menu items
            cursor.execute("DELETE FROM menu_items WHERE category != 'drink' AND category != 'beverage'")
            
        # 3. Drinks (Clear drink menu items)
        if 'drinks' in types:
            # First delete order items referencing these drinks (if orders weren't already cleared)
            if 'orders' not in types:
                cursor.execute('''
                    DELETE FROM order_items 
                    WHERE menu_item_id IN (SELECT id FROM menu_items WHERE category = 'drink' OR category = 'beverage')
                ''')
            # Now delete the menu items
            cursor.execute("DELETE FROM menu_items WHERE category = 'drink' OR category = 'beverage'")
            
        # 4. Waiters
        if 'waiters' in types:
            # Update orders to nullify waiter_id (if orders weren't already cleared)
            if 'orders' not in types:
                cursor.execute('UPDATE orders SET waiter_id = NULL WHERE waiter_id IN (SELECT id FROM waiters)')
            # Now delete waiters
            cursor.execute('DELETE FROM waiters')
            cursor.execute('DELETE FROM sqlite_sequence WHERE name = "waiters"')
        
        conn.commit()
        conn.close()
        
        return jsonify({'success': True, 'message': 'Selected data has been cleared successfully'})
...
if __name__ == '__main__':
    ...
    # -----------------------------------------------------------------------
    # System tray / control panel window
    # -----------------------------------------------------------------------
    root = tk.Tk()
    ...
    # Keep window on top so it's always visible
    root.attributes('-topmost', True)

    # Prevent closing via X — must use the Stop button
    def on_close():
        pass  # Do nothing; use the Stop button
    ...
    tk.Label(status_frame, text=' Server is running',
             font=('Segoe UI', 10), bg='#1e1e2e', fg='#a6adc8').pack(side='left')
    ...
    tk.Button(
        root, text='⏹  Stop Server',
        command=stop_server,
        font=('Segoe UI', 10, 'bold'),
        bg='#f38ba8', fg='#1e1e2e',
        relief='flat', padx=20, pady=6,
        cursor='hand2', activebackground='#eba0ac'
    ).pack()
