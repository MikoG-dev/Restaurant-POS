...
- **Advanced Backup Explorer**: Upgraded the interface with powerful filtering options (date range, payment method, and waiter) for deep auditing of preserved historical order data.
- **Licensing System**: Integrated secure license generation and verification modules (`license_check.py`, `generate_license.py`) to manage software distribution.
- **Standalone Executable Support**: Included PyInstaller build scripts (`build.py`, `RestaurantPOS.spec`) to easily compile the application into a standalone Windows executable.
...
```bash
git clone https://github.com/MikoG-dev/Restaurant-POS.git
cd Restaurant-POS
```
